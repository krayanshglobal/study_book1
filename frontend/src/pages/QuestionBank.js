...
import { CheckCircle2, XCircle, BookOpen, CalendarDays, CircleCheck, CircleDashed, ChevronDown, ChevronUp, Upload } from "lucide-react";
  ...
  const checkAnswer = async (q) => {
    ...
    try {
      ...
      if (r.data?.daily_bonus_earned) {
        toast.success("🎉 Daily Bonus! +1 point earned for completing today's questions!");
      }
      if (refresh) refresh();
    } catch (err) {
                    ...
                    {isOpen && (
                      ...
                      >
                        <div className="px-6 pb-5">
                          ...
                          {/* File / Image Answer Upload option */}
                          {q.allow_file_upload && (
                            <div className="mt-3 p-3 bg-purple-50/50 rounded-xl border border-purple-200/60">
                              <div className="flex items-center justify-between text-xs font-semibold text-[#0F1B4C] mb-1.5">
                                <span className="flex items-center gap-1.5">
                                  <Upload size={14} className="text-[#7C3AED]" /> Attach Solution File (.pdf, .jpg, .jpeg, .png)
                                </span>
                                {s.file_url && <span className="text-emerald-600 text-[11px] font-bold">✓ Attached</span>}
                              </div>
                              <input
                                type="file"
                                ...
                                onChange={async (e) => {
                                  ...
                                  try {
                                    ...
                                    setQ(q._id, { file_url: res.data.url, file_name: file.name });
