...
export default function StudentDashboard() {
  ...
  // Auto-play timer for dynamic moving announcements
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      const fetchUpcoming = api.get("/api/tests/upcoming").then((r) => setUpcoming(r.data.items || [])).catch(() => {});
      const fetchLb = api.get("/api/leaderboard", { params: { class_level: user?.class_level, limit: 5 } }).then((r) => setLb(r.data.items || [])).catch(() => {});
      const fetchRefs = api.get("/api/referrals/me").then((r) => setRefs(r.data)).catch(() => {});
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 sm:px-10 py-10">
        ...
        >
          ...
          {promos.length > 1 && (
            <>
              <button
                type="button"
                onClick={() => setActivePromoIndex((prev) => (prev - 1 + promos.length) % promos.length)}
                className="absolute left-3 top-1/2 -translate-y-1/2 z-30 w-9 h-9 rounded-full bg-[#0F1B4C]/50 hover:bg-[#0F1B4C] backdrop-blur-md border border-white/30 shadow-xl flex items-center justify-center text-white transition-all hover:scale-110"
              ...
              <button
                type="button"
                onClick={() => setActivePromoIndex((prev) => (prev + 1) % promos.length)}
                className="absolute right-3 top-1/2 -translate-y-1/2 z-30 w-9 h-9 rounded-full bg-[#0F1B4C]/50 hover:bg-[#0F1B4C] backdrop-blur-md border border-white/30 shadow-xl flex items-center justify-center text-white transition-all hover:scale-110"
            ...
            {promos.map((p, idx) => {
              ...
              return (
                <div key={p._id} className="w-full flex-shrink-0 flex flex-col">
                  {/* The Banner Card */}
                  <div
                    className={`rounded-3xl p-8 sm:px-12 pb-11 flex justify-between items-start md:items-center gap-6 relative overflow-hidden shadow-xl min-h-[220px] md:h-[220px] ${cfg.textColor} ${cfg.layoutClass}`}
                    style={{ background: cfg.gradient }}
                  >
                    {cfg.shapes}
                    {cfg.renderContent(p, user)}
                    {/* If banner has a custom image or PDF, render appropriate preview */}
                    {p.image_url ? (
                      p.image_url.includes("application/pdf") || p.image_url.endsWith(".pdf") ? (
                        <a
                          href={p.image_url}
                          target="_blank"
                          rel="noreferrer"
                          className="hidden md:flex flex-col items-center justify-center shrink-0 z-10 w-44 h-36 rounded-2xl bg-white/20 hover:bg-white/30 backdrop-blur-md border border-white/40 text-white p-3 text-center transition-transform hover:scale-105"
                        >
                          <span className="font-mono text-xs font-bold bg-red-600 px-2 py-0.5 rounded shadow-sm">PDF</span>
                          <span className="text-xs mt-2 font-semibold underline leading-tight">View Attachment</span>
                        </a>
                      ) : (
                        <div className="hidden md:flex shrink-0 z-10 items-center justify-center w-44 h-36 rounded-2xl overflow-hidden shadow-2xl border-2 border-white/40 bg-white/10">
      ...
      {/* Announcements — Side-by-side moving slider with navigation buttons */}
      {announcements.length > 0 && (
        <div className="mt-6 relative group">
          <div className="flex items-center justify-between gap-3 bg-[#0F1B4C] border border-[#0F1B4C] rounded-2xl px-5 py-3 shadow-md">
            <button
              onClick={() => setActiveAnnIndex((prev) => (prev === 0 ? announcements.length - 1 : prev - 1))}
              className="w-8 h-8 rounded-full bg-white/15 hover:bg-white/25 text-white grid place-items-center shrink-0 transition-colors cursor-pointer"
            ...
            <div className="flex-1 min-w-0 flex items-center gap-3 overflow-hidden">
              <div className="w-7 h-7 rounded-lg grid place-items-center bg-white/10 text-white shrink-0">
                <Bell size={14} className="animate-pulse" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] uppercase font-bold tracking-wider text-[#A78BFA] bg-white/10 px-2 py-0.5 rounded-full">
                    Announcement ({activeAnnIndex + 1}/{announcements.length})
                  </span>
                  <span className="text-[10px] text-white/50">
                    {new Date(announcements[activeAnnIndex]?.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}
                  </span>
                </div>
                <div className="font-semibold text-sm text-white truncate mt-0.5">
                  {announcements[activeAnnIndex]?.title}
                </div>
                {announcements[activeAnnIndex]?.body && (
                  <p className="text-xs text-white/80 line-clamp-2 mt-0.5 leading-snug">
                    {announcements[activeAnnIndex]?.body}
                  </p>
                )}
              </div>
