...
import { Plus, Trash2, Pencil, ClipboardCheck, ClipboardList, RefreshCw, ArrowLeft } from "lucide-react";
...
export default function ManageTests() {
  ...
  const [testLbStatus, setTestLbStatus] = useState({});
  ...
  const load = useCallback(async () => {
    ...
    await Promise.all(
      items.map(async (ft) => {
        try {
          const res = await api.get(`/api/admin/leaderboard/test/${ft._id}/release-status`);
          statuses[ft._id] = res.data.released;
        } catch {}
      })
    );
    setTestLbStatus(statuses);
  }, [activeClass]);

  const toggleTestLbRelease = async (testId) => {
    const current = !!testLbStatus[testId];
    try {
      const res = await api.post(`/api/admin/leaderboard/test/${testId}/release`, { released: !current });
      setTestLbStatus((prev) => ({ ...prev, [testId]: res.data.released }));
      toast.success(res.data.released ? "Test leaderboard released to students!" : "Test leaderboard hidden from students.");
    } catch (err) {
  ...
  const classesList = [
    { id: "all", label: "All Classes", desc: "View and manage tests across all grade levels", tone: "bg-purple-50 text-[#7C3AED] border-purple-200" },
    { id: "8", label: "Class 8", desc: "Grade 8 Sunday Mock & Final Exams", tone: "bg-blue-50 text-[#2563EB] border-blue-200" },
    { id: "9", label: "Class 9", desc: "Grade 9 Sunday Mock & Final Exams", tone: "bg-emerald-50 text-emerald-700 border-emerald-200" },
    { id: "10", label: "Class 10", desc: "Grade 10 Sunday Mock & Final Exams", tone: "bg-amber-50 text-amber-700 border-amber-200" },
  ];
    ...
    <div className="max-w-7xl mx-auto px-6 sm:px-10 py-10">
      ...
      {step === 1 ? (
        <div>
          ...
          <h1 className="mt-2 font-serif text-4xl text-[#0F1B4C] font-semibold">Manage Tests — Select Class</h1>
          <p className="mt-1 text-[#64748B] text-sm">Choose a class level to schedule tests or release test leaderboards.</p>
        ...
        <div>
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <div className="text-xs tracking-[0.24em] uppercase text-[#7C3AED] font-semibold">Manage Tests</div>
              <h1 className="mt-2 font-serif text-4xl text-[#0F1B4C] font-semibold">
                {activeClass === "all" ? "All Tests" : `Class ${activeClass} Tests`}
              </h1>
            </div>
                ...
                {showAddQ && (
                  <div className="mt-3 p-4 rounded-xl border border-[#7C3AED]/20 bg-[#7C3AED]/5 space-y-3">
                    ...
                    <div className="flex items-center gap-2 p-3 bg-purple-50/60 rounded-xl border border-purple-200">
                      <input
                        type="checkbox"
                        id="inline_allow_file_upload_cb"
                        checked={qForm.allow_file_upload || false}
                        onChange={(e) => setQForm((s) => ({ ...s, allow_file_upload: e.target.checked }))}
                        className="w-4 h-4 accent-[#7C3AED] rounded"
                      />
                      <Label htmlFor="inline_allow_file_upload_cb" className="cursor-pointer text-xs font-semibold text-[#0F1B4C]">
                        Enable optional file / image solution upload for student (.pdf, .jpg, .jpeg, .png)
                      </Label>
                    </div>
        ...
        {tests.map((t) => (
          <Card key={t._id} className="rounded-2xl border-slate-200 p-6" data-testid={`admin-test-card-${t._id}`}>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <Button
                size="sm"
                variant={testLbStatus[t._id] ? "outline" : "default"}
                className={testLbStatus[t._id] ? "rounded-full border-amber-300 text-amber-700 bg-amber-50" : "rounded-full bg-[#7C3AED] hover:bg-[#6D28D9] text-white"}
                onClick={() => toggleTestLbRelease(t._id)}
                data-testid={`test-release-lb-${t._id}`}
              >
                {testLbStatus[t._id] ? "Leaderboard Released (Hide)" : "Release Leaderboard"}
              </Button>
            </div>
          </Card>
        ))}
