...
from models import (
    ...
    FlashcardCreate, FlashcardUpdate, PromoBannerCreate, PromoBannerUpdate, now_iso
)
...
async def list_notes(
    ...
    if topic:
        q["topic"] = {"$regex": topic.strip(), "$options": "i"}
    if search:
        q["$or"] = [
...
async def list_discussions(
    ...
    async for doc in cursor:
        t = _ser(doc)
        if not t.get("student_id") and t.get("user_id"):
            try:
                u = await db.users.find_one({"_id": ObjectId(t["user_id"])}, {"student_id": 1})
                if u and u.get("student_id"):
                    t["student_id"] = u["student_id"]
            except Exception:
                pass
        t["replies_count"] = len(t.get("replies", []))
        # Keep replies so admin can see student messages
        threads.append(t)
...
async def reply_discussion(tid: str, body: DiscussionReplyCreate, user=Depends(get_current_user)):
    ...
    # If admin/superadmin replies, mark doubt thread as resolved so notification goes away
    if user.get("role") in ("admin", "superadmin"):
        upd["$set"] = {"resolved": True, "status": "resolved", "resolved_at": now_iso()}

    await db.discussion_threads.update_one({"_id": oid}, upd)
    return reply


@router.post("/api/discussions/{tid}/resolve")
async def resolve_discussion(tid: str, user=Depends(require_role("admin", "superadmin"))):
    from server import db
    oid = _safe_oid(tid, "thread ID")
    await db.discussion_threads.update_one(
        {"_id": oid},
        {"$set": {"resolved": True, "status": "resolved", "resolved_at": now_iso()}}
    )
    return {"ok": True}
...
async def list_flashcards(
    ...
    if topic and topic.strip().lower() not in ("all", ""):
        t = topic.strip()
        q["$or"] = [
            {"topic": {"$regex": re.escape(t), "$options": "i"}},
            {"topic": ""},
            {"topic": None},
            {"topic": {"$exists": False}}
        ]
    ...
    for item in items:
        f = _ser(item)
        if f.get("premium_only") and not is_premium:
            f["locked"] = True
            f["back"] = "Upgrade to premium to view answer."
        else:
            f["locked"] = False
        processed.append(f)

    return {"locked": False, "items": processed}
...
async def list_promos(class_level: Optional[str] = None):
    from server import db
    ...
    cursor = db.promos.find(q).sort("created_at", -1)
...
async def create_promo(body: PromoBannerCreate, user=Depends(require_role("admin", "superadmin"))):
    ...
    doc = {
        ...
        "class_level": body.class_level or "all",
        "created_at": now_iso(),
        "created_by": str(user["_id"])
    }
...
@router.put("/api/promos/{pid}")
async def update_promo(pid: str, body: PromoBannerUpdate, _=Depends(require_role("admin", "superadmin"))):
    from server import db
    ...
    doc = await db.promos.find_one({"_id": oid})
    if not doc:
        raise HTTPException(status_code=404, detail="Promo not found")
    return _ser(doc)


@router.post("/api/promos/upload")
async def upload_promo_asset(
    file: UploadFile = File(...),
    _=Depends(require_role("admin", "superadmin")),
):
    """Upload a banner asset (supports JPG, PNG, JPEG, PDF up to 5 MB)."""
    allowed_types = ["image/jpeg", "image/png", "image/jpg", "application/pdf"]
    ct = (file.content_type or "").lower()

    if not any(ct.startswith(at) or at in ct for at in allowed_types) and not file.filename.lower().endswith((".jpg", ".jpeg", ".png", ".pdf")):
        raise HTTPException(status_code=400, detail="Only JPG, PNG, JPEG, and PDF files are allowed.")
    ...
    mime = ct if ct in allowed_types else ("application/pdf" if file.filename.lower().endswith(".pdf") else "image/jpeg")
    data_uri = f"data:{mime};base64,{b64}"

    return {"url": data_uri, "filename": file.filename, "content_type": mime}
