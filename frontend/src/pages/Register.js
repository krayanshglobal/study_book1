...
export default function Register() {
  ...
  const submit = async (e) => {
    ...
    // 1. Phone validation: strictly 10 digits
    const cleanedPhone = form.phone.trim();
    if (!/^\d{10}$/.test(cleanedPhone)) {
      toast.error("Phone number must be strictly 10 digits (e.g. 9876543210)");
      return;
    }

    // 2. Email validation: valid format with @
    ...
    // 3. Password validation: min 8 chars, 1 caps, 1 special char
      ...
      <form onSubmit={submit} className="mt-10 space-y-5" data-testid="register-form">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Input id="email" type="email" data-testid="register-email-input" value={form.email} onChange={(e) => setField("email", e.target.value)} required className="mt-1.5 rounded-lg" placeholder="student@example.com" />
          </div>
          <div>
            <Label htmlFor="phone">Phone <span className="text-xs text-[#64748B]">(10 digits)</span></Label>
            <Input id="phone" data-testid="register-phone-input" value={form.phone} onChange={(e) => setField("phone", e.target.value.replace(/\D/g, "").slice(0, 10))} required maxLength={10} className="mt-1.5 rounded-lg font-mono" placeholder="9876543210" />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
          ...
          <div>
            <Label htmlFor="password">Password <span className="text-[10px] text-[#64748B]">(Min 8, 1 uppercase, 1 special)</span></Label>
            <Input id="password" type="password" data-testid="register-password-input" value={form.password} onChange={(e) => setField("password", e.target.value)} required minLength={8} className="mt-1.5 rounded-lg" placeholder="••••••••" />
          </div>
        </div>
        <div>
