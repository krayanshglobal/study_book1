...
_default_origins = [
    "https://student-managment-a4ef3.web.app",
    "https://student-managment-a4ef3.firebaseapp.com",
    "http://localhost:3000",
    "http://localhost:5173",
    "http://127.0.0.1:3000",
]
...
if cors_origins_str and cors_origins_str != "*":
    _origins = list(set([o.strip() for o in cors_origins_str.split(",") if o.strip()] + _default_origins))
else:
...
async def _startup():
    ...
    try:
        ...
        # Automatic student_id backfill for any existing users without student_id
        from routers.auth_routes import _generate_student_id
        async for u in db.users.find({"$or": [{"student_id": {"$exists": False}}, {"student_id": None}, {"student_id": ""}]}):
            cl = u.get("class_level") or "0"
