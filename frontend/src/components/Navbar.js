...
const adminLinks = [
  ...
  { to: "/admin/discussions", label: "Doubts" },
];
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-3 px-4 sm:px-6 py-2.5">
        <div className="flex items-center gap-3 shrink-0">
          <Link to={user ? (user.role === "student" ? "/dashboard" : "/admin") : "/"} className="shrink-0">
            <Logo size={44} showText={!user} />
          </Link>
        ...
        {user && (
          <nav className="hidden lg:flex items-center gap-0.5 shrink-0 overflow-x-auto">
            {links.map((l) => (
              <NavLink
                key={l.to}
                to={l.to}
                ...
                className={({ isActive }) =>
                  `px-2.5 py-1.5 rounded-full text-xs font-medium transition-colors duration-200 shrink-0 ${
                    isActive
                      ? "bg-[#0F1B4C] text-white font-semibold"
            ...
            <>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <div className="w-7 h-7 rounded-full overflow-hidden shrink-0 border border-slate-200 grid place-items-center bg-slate-100">
                      {user.avatar_url ? (
                        <img src={user.avatar_url} alt={user.name} className="w-full h-full object-cover" />
                      ) : (
                        <div
                          className="w-full h-full grid place-items-center text-white text-xs font-semibold"
                    ...
                    {user.student_id && (
                      <span className="hidden lg:inline-block font-mono text-[11px] font-bold bg-[#0F1B4C] text-white px-2.5 py-0.5 rounded-full border border-[#2563EB] shadow-xs">
                        {user.student_id}
                      </span>
                    )}
                  </button>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuLabel className="text-xs text-slate-500 flex flex-col gap-0.5">
                    <div>{user.email}</div>
                    {user.student_id && (
                      <div className="font-mono text-[#2563EB] font-bold">ID: {user.student_id}</div>
                    )}
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  {user.role === "student" && (
