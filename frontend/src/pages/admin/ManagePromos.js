...
import { Plus, Trash2, Calendar, Tag, Link as LinkIcon, Image as ImageIcon } from "lucide-react";

const empty = { title: "", subtitle: "", code: "", link_url: "", image_url: "", countdown_hours: 24, is_active: true, class_level: "all" };
...
export default function ManagePromos() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      fd.append("file", file);
      const r = await api.post("/api/promos/upload", fd, { headers: { "Content-Type": "multipart/form-data" } });
    ...
    <div className="max-w-7xl mx-auto px-6 sm:px-10 py-10">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage discounts, optional codes, links, images, and tickers displayed to students on their dashboard.</p>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between">
                  <Label>Banner Asset (Image / PDF)</Label>
                  <input
                    ref={fileInputRef}
                    type="file"
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="h-7 text-xs rounded-full gap-1 border-slate-300"
                    onClick={() => fileInputRef.current?.click()}
                    disabled={uploading}
                  >
                    <ImageIcon size={12} /> {uploading ? "Uploading…" : "Upload PDF/Image"}
                  </Button>
                </div>
                <Input
                  ...
                  placeholder="Paste URL or upload file (JPG, PNG, JPEG, PDF)"
                />
                {form.image_url && (
                  <div className="mt-2 w-full h-24 rounded-lg overflow-hidden border border-slate-200 bg-slate-50 flex items-center justify-center relative">
                    {form.image_url.includes("application/pdf") || form.image_url.endsWith(".pdf") ? (
                      <div className="flex flex-col items-center gap-1 text-slate-600">
                        <span className="font-bold text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded">PDF Document Attached</span>
                        <a href={form.image_url} target="_blank" rel="noreferrer" className="text-[11px] text-[#2563EB] underline">Preview PDF</a>
                      </div>
                    ) : (
              ...
              <div>
                ...
                <Select value={form.class_level || "all"} onValueChange={(v) => setForm({ ...form, class_level: v })}>
                  <SelectTrigger className="mt-1"><SelectValue placeholder="Target Audience" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">🌐 All Users & Classes</SelectItem>
      ...
      <div className="mt-8 grid md:grid-cols-2 gap-6">
        {items.map((promo) => (
          <Card key={promo._id} className="p-6 rounded-3xl border-slate-200 bg-slate-50 relative overflow-hidden flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-4 gap-2 flex-wrap">
                <div className="flex items-center gap-2">
                  <Badge className="bg-purple-600 text-white text-[10px]">
                    {promo.class_level === "all" || !promo.class_level ? "ALL CLASSES" : `CLASS ${promo.class_level}`}
                  </Badge>
                  {promo.code ? (
                    ...
                    <div className="text-xs text-slate-400 font-medium">No Code</div>
                  )}
                </div>
